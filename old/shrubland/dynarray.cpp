...
double stox(double* pd, std::string str) {return(stod(str));}
  float stox(float* pd, std::string str) {return(stof(str));}
      int stox(int* pd, std::string str) {return(stoi(str));}
    bool stox(bool* pd, std::string str) {return(bool(stoi(str)));}
uint16_t stox(uint16_t* pd, std::string str) {return(stoi(str));}
...
dynamic_array<T,S>::~dynamic_array() {
    std::cout << "\n dynarray deleted";
    if(ownership) {
        std::cout << "with ownership to heap";
        delete[] dyn_array;
    }
...
dynamic_array<T,S>::dynamic_array(const dynamic_array<T,S>& t)
    : dyn_array(t.dyn_array), ownership(t.ownership), j(t.j), n_row(t.n_row),
    n_col(t.n_col), sz(t.sz), j_col(t.j_col), j_row(t.j_row) {
